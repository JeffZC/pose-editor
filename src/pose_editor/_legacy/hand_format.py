"""
DEPRECATED – use `pose_editor.hand_format` instead.
"""
import warnings
from .. import hand_format as _new

warnings.warn(
    "pose_editor.hand_format_utils is deprecated; "
    "import pose_editor.hand_format instead.",
    DeprecationWarning,
    stacklevel=2,
)

__all__ = getattr(_new, "__all__", dir(_new))
globals().update({name: getattr(_new, name) for name in __all__})
